...
const (
	kubeStateMetricTemplate = `// Generated by *go generate* - DO NOT EDIT
/*
Copyright 2022. projectsveltos.io. All rights reserved.
...
// KubeStateMetrics namespace name
const Namespace = {{ printf "\"%s\"" .Namespace }}

// KubeStateMetrics deployment name
...
func generateKubeStateMetrics(filename, outputFilename, exportedName string) {
	...
	// Generate template.
	manifest := template.Must(template.New("kubestatemetrics-generate").Parse(kubeStateMetricTemplate))
...
func main() {
	prometheusConfigurationFile := "../kubestatemetrics/kube_state_metrics.yaml"

	generateKubeStateMetrics(prometheusConfigurationFile, "kube_state_metrics", "KubeStateMetrics")
}
...
func getNamespaceName(content string) string {
	...
	index := strings.Index(ns, namespace)
	if index == -1 {
		panic(fmt.Errorf("did not find Namespace name"))
	}

	var namespaceName string
	_, err := fmt.Sscanf(ns[index+len(namespace):], "%s", &namespaceName)
	if err != nil {
		panic(err)
	}
